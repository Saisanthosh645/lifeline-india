...
export function InteractiveMap({ onNodeClick, selectedNodeId, activeTab }: MapProps) {
  ...
  const filteredNodes = mapNodes.filter((node) => {
    ...
    if (activeTab === "Hospitals") return node.type === "hospital";
    if (activeTab === "Blood Banks") return node.type === "blood-bank";
    return true;
  });
  ...
  }, [leafletLoaded, coords.lat, coords.lng, selectedNodeId, activeSos.status, activeSos.step, activeBloodReservation?.elapsedSeconds, activeTab]);
    ...
    <div className="relative w-full aspect-[4/3] rounded-3xl overflow-hidden border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950 shadow-inner">
      ...
      <div className="absolute top-4 left-4 right-4 flex items-center justify-between pointer-events-none z-30">
        ...
        <div className="flex items-center gap-3 px-3.5 py-2 rounded-full text-[10px] font-bold text-zinc-600 bg-white/90 dark:text-zinc-300 dark:bg-zinc-900/90 border border-zinc-200 dark:border-zinc-800 shadow-lg backdrop-blur">
          ...
          {activeTab ? (
            <span className="ml-auto rounded-full bg-white/10 px-2 py-1 text-[9px] uppercase tracking-[0.35em] text-zinc-600 dark:text-zinc-300">Mode: {activeTab}</span>
          ) : null}
        </div>
      </div>
